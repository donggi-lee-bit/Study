...
- 인스턴스 생성 시 수행할 작업 (인스턴스 변수 초기화)에 사용
...
```
- 생성자도 오버로딩이 가능하다. 두 개의 생성자가 생성될 수 있는 이유는, 매개변수가 다르기 때문이다.

```
Card c = new Card();
```
- 지금까지 new Card(); 의 Card()는 클래스 이름인 줄 알았는데, 이건 생성자를 생성해주는 것이었다.
- 그 위의 코드처럼 생성자를 만들어주지 않아도 지금까지 생성자를 불러올 수 있었던 것은 기본 생성자라고 하는 것이 안 만들어도 생기기 때문이었다.
...
## 변수의 초기화
- 지역변수는 수동 초기화 해줘야 한다
- 멤버변수(인스턴스 변수, static 변수)는 자동 초기화된다

```java
class InitTest {
    int x;          // 인스턴스 변수 -> 자동 초기화가 된다
    int y = x;      // 인스턴스 변수
    
    void method1() {
        int i;      // 지역변수 -> 수동 초기화 해줘야함
        int j = i;  // 에러, 지역변수를 초기화하지 않고 사용했다
    } 
} 
```
